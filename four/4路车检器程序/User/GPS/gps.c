...
void GPS_display(void)
{
  ...
  { 
	...
	if(t>0 && t<20)
	{
		g_timer[3] = (GPS_Type.GPS_t[0]-0x30)*10+(GPS_Type.GPS_t[1]-0x30);		//时
		g_timer[4] = (GPS_Type.GPS_t[3]-0x30)*10+(GPS_Type.GPS_t[4]-0x30);		//分
		g_timer[5] = (GPS_Type.GPS_t[6]-0x30)*10+(GPS_Type.GPS_t[7]-0x30);		//秒	
	}
  
		  
}
void GPS_J(void)
{    Gps_signal = (GPS_Type.GPS_sv[0]-0x30)*10 + GPS_Type.GPS_sv[1]-0x30;
		 g_timer[3] = (GPS_Type.GPS_t[0]-0x30)*10+(GPS_Type.GPS_t[1]-0x30);		//时
		 g_timer[4] = (GPS_Type.GPS_t[3]-0x30)*10+(GPS_Type.GPS_t[4]-0x30);		//分
		 g_timer[5] = (GPS_Type.GPS_t[6]-0x30)*10+(GPS_Type.GPS_t[7]-0x30);		//秒			 
